...
<nav class="main-nav" class:js-enabled={javascript} aria-label="Hoofdmenu">
  <a href="/" class="logo">
...
<!-- MOBILE OVERLAY (altijd renderen!) -->
<div
  bind:this={overlayEl}
...
<style>
  ...
  .menu-button, .menu-link { cursor: pointer; font-weight: 600; background: none; border: none; }
  .menu-button:hover, .menu-link:hover { color: var(--highlight-color); }
  .dropdown { position: absolute; top: 100%; left: 0; background: var(--background-color); border-radius: 0 0 .4rem .4rem; padding: 0.5rem 0; box-shadow: 0 6px 12px rgba(0,0,0,.1); }

  /* MOBILE */
  .hamburger { display: none; flex-direction: column; gap: 0.4rem; background: none; border: none; cursor: pointer; }
  .hamburger span { width: 28px; height: 3px; background: var(--text-color); }

  .mobile-overlay {
    position: fixed; top: 0; left: 0; width: 100%; height: 100%;
    flex-direction: column; padding: 2rem;
    background: var(--background-color); z-index: 200;
    display: flex;
    transform: translateY(-100%);
  }

  .mobile-overlay.open {
    /* optioneel: CSS fallback animatie als GSAP niet gebruikt wordt */
    transform: translateY(0%);
  }

  .close-btn { align-self: flex-end; font-size: 3rem; background: none; border: none; cursor: pointer; }
  .overlay-menu { margin-top: 3rem; display: flex; flex-direction: column; gap: 1.5rem; }
  .overlay-button, .overlay-item a { font-size: 2rem; font-weight: 700; background: none; border: none; text-align: left; color: var(--text-color); }
  .overlay-item a:hover { color: var(--highlight-color); text-decoration: underline; }
  .overlay-dropdown { padding-left: 1rem; display: flex; flex-direction: column; gap: 0.75rem; }
